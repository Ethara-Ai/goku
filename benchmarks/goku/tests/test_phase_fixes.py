...
def _refused_result(item_type: str, points: int) -> ScorerResult:
    ...
    item = RubricItem(
        number=1,
        type=item_type,  # type: ignore[arg-type]  # test helper takes any rubric type str
        category="MM_REASONING" if points > 0 else "HALLUCINATION",
        points=points,
        importance="mandatory",
        criterion="x",
...
from benchmarks.goku.scorers.llm_judge import (  # noqa: E402
...
def test_score_llm_judge_no_retry_when_clean():
    ...
    clean = ScorerResult(
        number=1,
        passed=True,
        judge_rationale="Agent did fine.",
        points_awarded=5,
        judge_cost_usd=0.01,
    )
    with _mock.patch(
...
def test_score_llm_judge_vote_majority_pass_when_suspicious():
    ...
    with _mock.patch(
        ...
        r = score_llm_judge_wrapped(
            item=item,
            response="r",
            file_contents="meal_log.json content",
            trajectory="[6] ActionEvent\n  Command: create",
            judge_model="gemini/gemini-3.5-flash",
        )
...
from benchmarks.goku.task_loader import (  # noqa: E402
...
def test_load_items_from_scores_jsonl_inverts_negative_items():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with open(scores_path, "w") as f:
            ...
            f.write(
                _json.dumps({"number": 1, "passed": True, "judge_rationale": "ok"})
                + "\n"
            )
            # Negative item: display passed=False → internal passed=True (hallucination matched)
            f.write(
                _json.dumps(
                    {"number": 2, "passed": False, "judge_rationale": "hallucinated"}
                )
                + "\n"
            )
...
def test_load_items_from_scores_jsonl_skips_summary_rows():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with open(scores_path, "w") as f:
            f.write(
                _json.dumps({"number": 1, "passed": True, "judge_rationale": ""}) + "\n"
            )
            f.write(_json.dumps({"pass": True}) + "\n")
            f.write(_json.dumps({"per_task_score": 1.0}) + "\n")
            f.write(
                _json.dumps({"awarded": 5, "max_total": 5, "raw_score": 1.0}) + "\n"
            )
...
def test_is_archive_path_matches_directory_suffix():
    ...
    assert _is_archive_path(
        Path(
            "eval_outputs/run_1/goku/claude-opus-4.7_sdk_test/"
...
def test_is_archive_path_rejects_live_paths():
    ...
    assert not _is_archive_path(
        "eval_outputs/run_1/goku/claude-opus-4.7_sdk_test/task_20321/scores.jsonl"
    )
...
def test_rescore_discover_targets_skips_archives(tmp_path):
    ...
    (
        model_dir / "task_xyz.archive_pre_rerun_20260521_000000" / "scores.jsonl"
    ).write_text(_json.dumps({"pass": False}) + "\n", encoding="utf-8")
...
def test_double_negative_response_not_criteria_is_flagged():
    ...
    from benchmarks.goku.task_loader import (
        _looks_like_double_negative_response_not_criteria,
    )
...
def test_judge_prompt_uses_named_constants_not_hardcoded_caps():
    ...
    assert (
        "_PROMPT_RESPONSE_MAX_CHARS" in src or "_PROMPT_FILE_CONTENTS_MAX_CHARS" in src
    ), (
...
def test_judge_accepts_large_text_payload_under_new_cap(monkeypatch):
    ...
    result = llm_judge._score_llm_judge_single(
        item=item,
        response="r",
        file_contents=big_file_contents,
        trajectory="t",
        judge_model="gemini/gemini-3.5-flash",
        judge_api_key="dummy",
    )
...
def test_update_output_jsonl_missing_file_returns_false(tmp_path):
    ...
    ts = _build_task_score()
    assert (
        update_output_jsonl_test_result(tmp_path / "absent.jsonl", "task_x", ts)
        is False
    )
...
def test_update_output_jsonl_unknown_instance_returns_false(tmp_path):
    ...
    # File is untouched
    assert (
        _json.loads(out.read_text(encoding="utf-8").strip())["instance_id"] == "task_a"
    )
...
def test_litellm_opus_47_env_var_adds_extra_ids(monkeypatch):
    ...
    assert (
        AnthropicModelInfo._is_claude_4_7_model(
            "bedrock/converse/arn:.../custom_pid_42"
        )
        is True
    )
    assert (
        AnthropicModelInfo._is_claude_4_7_model("bedrock/converse/arn:.../other_pid_99")
        is True
    )
    # Unknown ID is still rejected.
    assert (
        AnthropicModelInfo._is_claude_4_7_model("bedrock/converse/arn:.../unknown_pid")
        is False
    )
...
def test_agent_video_keyframes_count_is_at_least_120():
    ...
    src = Path(str(_REPO_ROOT / "benchmarks/goku/run_infer.py")).read_text()
...
def test_keyframes_are_jpeg_not_png():
    ...
    assert '"-q:v"' in src and '"3"' in src, "JPEG quality not pinned at 3"
...
def test_judge_total_bytes_cap_skips_excess_images(tmp_path):
    ...
    try:
        ...
        assert 1 <= len(blocks) < len(paths), (
            f"expected partial blocks under cap; got {len(blocks)} of {len(paths)}"
        )
...
def test_run_batch_timeout_default_is_at_least_3600s():
    ...
    src = Path(str(_REPO_ROOT / "run_batch.sh")).read_text()
...
def test_judge_video_anthropic_skips_native_upload(tmp_path):
    ...
    with (
        patch.object(llm_judge, "_upload_video_to_gemini") as up,
...
def test_judge_video_gemini_happy_path_emits_file_block(tmp_path):
    ...
    try:
        with (
            ...
            patch.object(llm_judge, "video_to_keyframes") as kf,
        ):
            blocks, warnings = llm_judge._build_media_blocks(
                [str(fake_video)],
                judge_model="gemini/gemini-3.5-flash",
...
def test_judge_video_missing_google_genai_raises_with_install_hint(tmp_path):
    ...
    sys.modules["google"] = _Blocker()  # type: ignore[assignment]  # import-blocking stub
    try:
        try:
            llm_judge._upload_video_to_gemini(fake_video, "fake-key")
        except RuntimeError as exc:
            assert "google-genai" in str(exc), f"error must hint at the fix; got: {exc}"
...
def test_rescore_main_eagerly_applies_httpx_patches():
    ...
    src = Path(str(_REPO_ROOT / "benchmarks/goku/rescore.py")).read_text()
    m = re.search(
        r"^def main\(\) -> None:\n([\s\S]+?)\n(?:^\S|^def\s)", src, re.MULTILINE
    )
...
def test_sdk_patches_uses_discriminated_union():
    ...
    src = Path(str(_REPO_ROOT / "benchmarks/utils/sdk_patches.py")).read_text()
...
def _run_infer_src() -> str:
    return Path(str(_REPO_ROOT / "benchmarks/goku/run_infer.py")).read_text()
...
def test_run_batch_kill_ghost_still_exists():
    ...
    # time-bounded.
    m = re.search(r"kill_ghost_containers\(\) \{([\s\S]+?)^\}", src, re.MULTILINE)
...
from benchmarks.goku import pdf_pipeline as _pdf_pipeline  # noqa: E402


_PDF_04_DIR = (
    Path(__file__).resolve().parents[3] / "dataset" / "pdf_04" / "data" / "input_files"
)
...
def test_pdf_pipeline_thumbnails_are_small():
    ...
    if not _pdf_fixtures_available():
        return
    thumbs = _pdf_pipeline._render_thumbnails(
        _WRANGLER_PDF, dpi=_pdf_pipeline.THUMBNAIL_DPI
    )
...
def test_run_infer_pdf_tool_mode_default():
    ...
    for line in src.splitlines():
        if "GOKU_PDF_MODE" in line and "os.environ" in line and "tool" in line:
            assert '"tool"' in line, (
                f"default for GOKU_PDF_MODE must be 'tool' (not inline); got: {line!r}"
            )
            return
...
def test_judge_max_tokens_fits_gemini_thinking_budget():
    ...
    src = Path(str(_REPO_ROOT / "benchmarks/goku/scorers/llm_judge.py")).read_text()
...
def test_shell_scorer_subdir_fallback_shallowest_wins_among_decoys():
    """The real rescue (file one level deeper) must still PASS even when many
    deeper decoy copies of the same filename exist. Shallowest-first ordering
    reaches the passing copy before the retry cap."""
    from benchmarks.goku.models import RubricItem
    from benchmarks.goku.scorers.deterministic import _score_shell_succeeds_real

    with tempfile.TemporaryDirectory() as td:
        outdir = Path(td)
        # Passing copy one level deep.
        (outdir / "results").mkdir()
        (outdir / "results" / "data.json").write_text('{"x": 1}')
        # Many deeper decoys with the WRONG value (would fail the assertion).
        for i in range(12):
            deep = outdir / f"d{i}" / "nested" / "more"
            deep.mkdir(parents=True)
            (deep / "data.json").write_text('{"x": 999}')
        item = RubricItem(
            number=1,
            type="shell_succeeds_real",
            category="CORRECTNESS",
        ...
        passed, rationale = _score_shell_succeeds_real(item, outdir, "")
        assert passed is True, f"shallow rescue should win: {rationale}"
        assert "results" in rationale


def test_shell_scorer_subdir_fallback_is_capped_and_noted():
    """An organize-style task that copied an identically-named file into far
    more subdirs than the cap must NOT fan out unboundedly. The fan-out is
    bounded to _MAX_SUBDIR_FALLBACK_RETRIES and the truncation is surfaced
    in the rationale so the FAIL is not silently incomplete."""
    import time

    from benchmarks.goku.models import RubricItem
    from benchmarks.goku.scorers import deterministic as det
    from benchmarks.goku.scorers.deterministic import _score_shell_succeeds_real

    with tempfile.TemporaryDirectory() as td:
        outdir = Path(td)
        n = det._MAX_SUBDIR_FALLBACK_RETRIES + 6
        for i in range(n):  # all WRONG value → none can pass
            sub = outdir / f"sub{i:02d}"
            sub.mkdir()
            (sub / "data.json").write_text('{"x": 999}')
        item = RubricItem(
            number=1,
            type="shell_succeeds_real",
            category="CORRECTNESS",
        ...
        assert passed is False
        # Bounded fan-out: only the cap's worth of subdirs were retried, each
        # of which fails fast — so this finishes well under an unbounded run.
        assert elapsed < 30
        assert str(det._MAX_SUBDIR_FALLBACK_RETRIES) in rationale
        assert "retried" in rationale
...
def test_p1_ever_seen_preserves_entries_across_strip_cycles():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # Strip A.
        strip_task_from_jsonl(
            live, {"task_a"}, dry_run=False, archive_suffix=".archive_test1"
        )
        ...
        # Now strip B.
        strip_task_from_jsonl(
            live, {"task_b"}, dry_run=False, archive_suffix=".archive_test2"
        )
        # Live is empty.
